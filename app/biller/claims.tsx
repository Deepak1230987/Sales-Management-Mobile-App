...
export default function ClaimsManagement() {
              ...
              .map((doc) => {
                ...
                if (data.createdAt) {
                  if (
                    ...
                    try {
                      claimDate = data.createdAt.toDate();
                    } catch (error) {
                      console.warn("Error converting createdAt:", error);
                      claimDate = new Date();
                    }
                  } else if (data.createdAt.seconds) {
                    claimDate = new Date(data.createdAt.seconds * 1000);
                  }
                } else if (data.date) {
                  if (
                    data.date.toDate &&
                    ...
                    try {
                      claimDate = data.date.toDate();
                    } catch (error) {
                      console.warn("Error converting date:", error);
                      claimDate = new Date();
                    }
                  } else if (data.date.seconds) {
                    claimDate = new Date(data.date.seconds * 1000);
                  }
                }
  ...
  const handleClaimPress = (claimId: string) => {
    router.push(`/biller/claim-details?claimId=${claimId}`);
  };
        ...
        <View style={styles.quickLinksContainer}>
          ...
          <View style={styles.quickLinks}>
            <TouchableOpacity
              ...
              onPress={() => router.push("/biller/claim-form?claimId=new")}
            >
              <View
            ...
            <TouchableOpacity
              style={styles.quickLink}
              onPress={() => router.push("/biller/prizes")}
            >
              <View
                ...
                <View style={styles.emptyContainer}>
                  ...
                  {!searchQuery && (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        router.push("/biller/claim-form?claimId=new")
                      }
                    >
