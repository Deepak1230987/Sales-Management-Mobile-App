...
export default function EditSale() {
  ...
  const handleSave = async () => {
    try {
      ...
      console.log("phoneNumber:", updatedSaleData.phoneNumber); // Apply stock adjustments for edited items
      try {
        await adjustStockForEditedSale(originalSaleData?.items || [], items);
        console.log("✅ Stock adjustments applied successfully");
      } catch (stockError) {
        console.error("❌ Error adjusting stock:", stockError);
        Alert.alert(
          "Warning",
          "Sale updated but stock adjustment failed. Please check inventory manually."
        );
      }
